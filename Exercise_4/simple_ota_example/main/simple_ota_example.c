...
void simple_ota_example_task(void * pvParameter) {
	...
	esp_http_client_config_t config = { .url = CONFIG_FIRMWARE_UPGRADE_URL,
			.cert_pem = (char *) server_cert_pem_start, .event_handler =
					_http_event_handler };
	esp_err_t ret = esp_https_ota_mod(&config);
	if (ret == ESP_OK) {
		esp_restart();
	} else {
